...
function PlaceDetails() {
	...
	useEffect(() => {
		const fetchData = async () => {
			const response = await fetch(`http://localhost:5006/places/${placeId}`)
	...
	async function deletePlace() {
		await fetch(`http://localhost:5006/places/${place.placeId}`, {
			method: 'DELETE'
		})
		history.push('/places')
	}
	...
	async function deleteComment(deletedComment) {
		await fetch(`http://localhost:5006/places/${place.placeId}/comments/${deletedComment.commentId}`, {
			method: 'DELETE'
		})
	...
	async function createComment(commentAttributes) {
		const response = await fetch(`http://localhost:5006/places/${place.placeId}/comments`, {
			method: 'POST',
			headers: {
